...
namespace
{
	const char* const WINDOW_TITLE = "Emily Wood 7-1 Project"; // Macro for window title
...
int main(int argc, char* argv[])
{
	...
	texFilename = "resources/textures/spoon.jpg";
	if (!UCreateTexture(texFilename, textureSpoonId))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}

	texFilename = "resources/textures/coffeebag.jpg";
	if (!UCreateTexture(texFilename, textureBagId))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
...
void URender()
{
	...
	if (isPerspective)
	{
		...
		view = glm::translate(glm::vec3(0.0f, 0.0f, -10.0f));
	...
	*/
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(4.0f, 0.0f, 2.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(4.0f, 2.05f, 2.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(5.0f, 1.0f, 2.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	*/
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0.5f, 0.0f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	*/
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0.5f, 3.75f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	*/
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(2.0f, 1.25f, -0.75f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	*/
	...
	// 3. Position the object
	translation = glm::translate(glm::vec3(-0.875f, 3.5f, -1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	*/
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, textureSpoonId);

	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.125f, 3.0f, 0.125f));
	// 2. Rotate the object
	glm::mat4 rotation1 = glm::rotate(glm::radians(90.0f), glm::vec3(0.0, 0.0f, 1.0f));
	glm::mat4 rotation2 = glm::rotate(glm::radians(210.0f), glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-1.75f, 0.125f, 5.5f));
	// Model matrix: transformations are applied right-to-left order
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.5f, 0.125f, 0.75f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(120.0f), glm::vec3(0.0, 1.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(1.5f, .09375f, 3.625f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	*/
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, textureBagId);

	// 1. Scales the object
	scale = glm::scale(glm::vec3(2.5f, 4.5f, 1.0f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(45.0f), glm::vec3(0.0, 1.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-3.5f, 2.25f, 3.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glProgramUniform4f(gSurfaceProgramId, objColLoc, 0.5f, 0.5f, 0.0f, 1.0f);
	...
	//// 1. Scales the object
	//scale = glm::scale(glm::vec3(2.9f, 3.9f, 1.0f));
	//// 2. Rotate the object
	//rotation = glm::rotate(glm::radians(45.0f), glm::vec3(0.0, 1.0f, 0.0f));
	//// 3. Position the object
	//translation = glm::translate(glm::vec3(-3.49f, 2.0f, 0.0f));
	//// Model matrix: transformations are applied right-to-left order
	//model = translation * rotation * scale;
	//glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
*/
	...
	*/
	...
	glUniform3f(light1PosLoc, 0.0f, 2.0f, 1.0f);
